...
def _text_input_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--glosser", choices=["simple", "spacylemma", "rules"], required=True)
...
def _validate_language_pair(spoken_language: str, signed_language: str):
    valid_pairs = {("fr", "fsl"), ("en", "ase")}
    if (spoken_language, signed_language) not in valid_pairs:
        pairs_str = ", ".join([f"{sp}/{si}" for sp, si in sorted(valid_pairs)])
        raise SystemExit(f"Unsupported language pair {spoken_language} -> {signed_language}. Valid pairs: {pairs_str}.")
...
def text_to_gloss():
    ...
    _validate_language_pair(args.spoken_language, args.signed_language)
